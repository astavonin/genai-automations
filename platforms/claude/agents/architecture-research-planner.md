...
**The rule outranks the list, which is non-exhaustive by construction** — banning one token yields substitutes. A zero count is necessary, not sufficient: rewriting a listed token as "the choice here was made advisedly" leaves the count clean and the rule broken. Three exemptions the tool applies for you: a token inside a fenced block; a token wrapped in backticks or quotes — a mention rather than a use, which is how a document discusses the rule; and a `## Prior Context` section in `analysis.md`, skipped entirely because its rows are `/research`'s pasted `search docs` locator table, not prose you authored. The same heading in any other file, or any other heading in `analysis.md` — including the findings prose you write yourself — counts and scans as normal. A token merely sitting inside a longer quoted sentence is still a use and still counts. Table cells and blockquotes are **not** exempt: a defensive sentence is defensive wherever it sits, and both were otherwise a way to satisfy the gate without changing the prose.
...
7. **Measure the prose you just wrote and report the numbers.** Run the tool on every Markdown file you created or modified and quote the results back in your response:
   ...
   - **Design-field counters — the lines printed after `UNSLOTTED:` — only a `design.md` is gated on these.** They check §7 `**Cost:**`/`**Misses:**` presence and §3 `From:` tag presence. `/verify-docs` classifies which of them block versus warn (see its Step 2 table); resolve every blocker before returning. A document written before these fields existed can carry a non-zero warning, but a `design.md` you are writing now should read zero.
