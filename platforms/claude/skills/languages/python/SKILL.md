...
- Follow AAA pattern (Arrange, Act, Assert); name tests after the behavior and expected outcome.
- Test edge cases: empty input, `None` values, boundary conditions, and error paths.

## Verification

- Run project-native formatting, linting, typing, packaging, and test commands first.
- Apply Ruff (or the project-equivalent formatter and linter) to every modified file. Ruff does not detect type errors; a separate type checker is required.
