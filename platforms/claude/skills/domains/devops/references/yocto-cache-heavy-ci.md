...
- `interruptible` → `cancel-in-progress` on concurrency group
...
- Track start time in a file when `after_script` needs it. Do not depend on shell variables surviving CI phase boundaries (e.g. GitLab `script` → `after_script`, or between GitHub Actions steps).
