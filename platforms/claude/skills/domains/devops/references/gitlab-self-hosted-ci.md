...
- Use `projctl` for issues, MRs, comments, sync, and pipeline metadata. Check `projctl --help` and subcommand help before raw alternatives. Extend `projctl` (source at `~/projects/projctl`) rather than bypassing it with `glab`, `gh`, or direct API calls.
...
- Audit local and CI Docker build flags together. `Makefile`, local build scripts, and CI build jobs must agree on `--platform`, cache source, `BUILDKIT_INLINE_CACHE`, `--provenance`, `--sbom`, Dockerfile path, secrets, and build args unless a difference is documented.
...
- Use `projctl` first for pipeline, job, and MR queries. Do not invent unsupported subcommands, and extend the wrapper when the required operation is truly missing.
