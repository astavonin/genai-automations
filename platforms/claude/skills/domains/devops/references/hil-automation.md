...
- Treat runnable HIL/device verification as a completion gate. Do not mark device-verifiable implementation work complete until the relevant device/HIL job, verifier CLI, or scenario set has passed.
...
- When project guidance or the task spec already names an entry point, use that exact entry point rather than inventing a substitute.
- When a design or task spec lists concrete device verification commands, run them as part of implementation verification; do not leave them as instructions for the user.
...
- Has runnable device/HIL verification actually been run for device-verifiable work?
