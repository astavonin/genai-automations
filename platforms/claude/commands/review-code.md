...
**Then pack ALL implementation files into one bundle** — in the main conversation, before launching any reviewer agent: `bash ~/.claude/scripts/review-pack.sh "<scratchpad>/review-bundle.txt" <file>…`. That one name is the round's bundle: every agent in the round reads the same file, and the next round's pack replaces it. Pass the bundle *path* and the packer's printed `lines` count in every agent prompt.

**Read it to the end.** Instruct each agent to read the bundle from its first line to the reported line count, in successive `offset`/`limit` calls where one call returns less, before grading anything. The Read tool returns 2,000 lines by default and a bundle routinely runs past that, so an agent that stops at the first return grades a fraction and reports findings as complete. A call that errors with a token-limit message (`exceeds maximum allowed tokens`) is not a short return — it means the window is still too large: halve the `limit` and retry from the same `offset`, halving again on each further token-limit error, until a call returns content. Decline only once the bundle is still unreachable after repeatedly halving the window. An agent that cannot reach the reported line count says so in its output instead of returning findings.

**What an agent may Read as evidence:** the bundle path, and nothing else — no repository file is read as evidence. Every other repository path costs an approval prompt, which is why everything else reaches agents through the prompt. Exempt: the loads `agents/reviewer.md` mandates on every review regardless of subject — the review checklist and the reviewer's memory directory — since neither is evidence and neither costs an approval prompt.

**How the bundle is framed:** each member is preceded by a line reading `===== review-pack: <path> =====` at column 0, and every line after such a header belongs to that file until the next one. The packer refuses any input carrying that marker at column 0, so a header inside the bundle is always one the packer wrote.

If the packer exits non-zero, fix the input list and re-run. Pasting contents inline as a fallback puts back the payload this removes.

Typical files to pack:
...
- **Interface files not in the diff:** for each changed `.cc`/`.cpp`/`.c` file, also pack its `.h`/`.hpp` if it exists and is not already in the diff; for Go, pack the interface definition files the changed package implements
- **Full design doc** (`planning/<goal>/milestone-XX/issues/<NNN-name>/design.md`) if one exists — pack the entire file, not just the acceptance criteria section

**`analysis.md`** if one exists — quote `## Change Class` and the design header's `**Class:**` into every prompt: the orchestrator transcribes the first into the Codex review-request anyway, and agents corroborate the two quoted values against each other. Quote `## On-Device Scope` too — Review Scope below has every agent raise a Critical finding where on-device scope is YES or YES-UNKNOWN and the design's On-Device Verification section is absent, and a check whose input never arrives still reports APPROVED.

**On-device entry-point existence check (orchestrator, not agents):** When `## On-Device Scope` is YES or YES-UNKNOWN and the design doc's On-Device Verification section names a concrete `**Entry point:**` (not a template placeholder), the orchestrator — not any agent — checks in the main conversation's shell whether that script or Makefile target exists on disk, and quotes the result (`entry point exists on disk` / `entry point does not exist on disk` / `entry point field is a template placeholder — not checked`) into every prompt alongside the `## On-Device Scope` block. This is the one disk fact the review needs that is not evidence content, so it is resolved once upstream instead of by each agent separately.

**The observed-failure ledger** (`<issue-folder>/observed-failures.md`, resolved per `~/.claude/skills/workflows/issue-folder-resolve/SKILL.md`) if one exists — mandatory when present, stays orchestrator-read and is pasted whole into every prompt's fence. Agents are instructed to validate its entries and waivers but may not Read it themselves; without it inline they will report a valid, user-approved waiver as a missing test, producing a finding no coder can fix. If it does not exist, say so explicitly in the prompt rather than omitting the topic.
...
**Bundle for Codex.** The Codex review-request is a separate channel — `codex_flow/prompting.py` → `build_review_prompt()` forwards the request document and nothing else — so the bundle reaches Codex only if the request names it. The request's `## Context` section gains a `**Review bundle:**` line carrying the bundle's absolute path and its `lines` count, beside the template's existing `**Design doc:**` line, and the request instructs Codex to read it to that count as the reviewed text. `--sandbox read-only` restricts writes, not reads, and `--cd` sets the working directory, so a review run reaches a bundle in the session scratchpad.
...
1. Run the **Consensus Review Protocol** (Steps A–H) against the implementation
   ...
   - **Every Claude agent prompt must include** the bundle path and its `lines` count, the two quoted `analysis.md` sections (plus the orchestrator-supplied entry-point existence result when on-device scope applies), the ledger, and the review checklist — the checklist carries Test Quality Pass Step 3 and its severity table, so no separate paste is needed. See the Observed-Failure Regression Requirement section below.
...
- **On-Device Verification entry point:** Use the `## On-Device Scope` block and the orchestrator-supplied entry-point existence result, both quoted in the prompt — do not check the filesystem yourself; the orchestrator resolves that disk fact once, upstream, per Setup. If on-device scope is YES or YES-UNKNOWN and the design doc's On-Device Verification section is absent, flag that absence as a Critical finding for design adherence, regardless of the existence result. If the quoted result is `entry point field is a template placeholder — not checked`, flag as High immediately. If the quoted result is `entry point does not exist on disk`, flag as High. If the entry-point was listed as a deliverable, verify it is implemented and covers the documented build/deploy/verify steps, and that it reflects the expected outcomes and failure indicators from the design doc. Flag as High if the entry-point is missing or incomplete; flag as Critical if the design doc section is absent despite confirmed on-device scope.
