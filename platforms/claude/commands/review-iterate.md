...
- **Do not self-serve a waiver.** If you judge that a category holds, including category 6, flag the finding instead of fixing it — use the declined-finding pause below; the user decides outside the loop. Every recorded waiver carries `## Waiver`'s full requirements — the category, the user's approval under the approval test there, and a compensating control.
...
- Code review: invoke coder agent scoped to this finding only, explicitly passing the test requirements above and the resolved `<issue-folder>` path (per `~/.claude/skills/workflows/issue-folder-resolve/SKILL.md`). Per `~/.claude/skills/workflows/regression-test/SKILL.md` → The Ledger → Who writes it, this loop appends an entry for any defect that reproduces mid-loop; if the finding describes incorrect runtime behaviour that the coder confirms reproduces, it must append a resolved entry to `<issue-folder>/observed-failures.md`. Without the path the coder cannot write it, and Step 2c's scoped verification of the test obligation has nothing to check.
...
- The coder's fix response for this finding — the record of which clause or waiver, if any, it claims as a discharge
- Output of `git diff` since the review baseline (or since the last commit)
- Instruction: "Verify ONLY whether finding [ID] has been resolved. Do not expand scope beyond the changed lines. Return one of: RESOLVED / STILL OPEN / REGRESSION INTRODUCED. If REGRESSION INTRODUCED: describe what broke and where. For Critical and High **code-review** findings, in addition to verifying the fix resolves the finding, the test obligation is met by a test change in the diff, by a fix response naming an `### Out of Scope` clause, or by a fix response recording a waiver the user approved; return RESOLVED only when the finding's concern is gone and the test obligation is met. Return STILL OPEN when the finding's concern survives, or when none of the three is present. For Critical, High, and Medium **design-review** findings: verify the design.md change addresses the architectural concern named in the finding — return STILL OPEN if only cosmetic edits were made or the concern remains visible in the updated section. **Deletion or rewriting of the offending text is a valid resolution** — verify the concern is gone, not that text was added; a fix that removes the ambiguous passage resolves the finding. But if the fix deleted a section required by `DESIGN-TEMPLATE.md`, or left any surviving section referencing the removed content, return STILL OPEN."

`tests/verify-config-consistency.sh` checks that Step 2b and this step carry the discharge wording above and that `STILL OPEN` occurs nowhere in this file outside Step 2c.
...
For a code review, pass the per-finding fix responses collected in Step 2 alongside the prior `code-review.md` — each records, per finding ID, the clause or approved waiver claimed as a discharge. Where the final re-review accepts a discharge for a Critical or High finding, record the accepted clause or waiver on that finding's resolution line in `code-review.md`.
...
**Stop. Do not automatically start a new fix-verify loop. For code-review runs, do not proceed to `/verify` automatically. For design-review runs, do not proceed to `/implement` automatically. See CLAUDE.md Critical Rules for the two-part test.**
