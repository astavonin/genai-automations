...
- **Skip** the design doc status header update step (`**Status:** Draft → Approved`) — the fix-loop manages the header itself and only sets it after the final clean review
...
**Which findings to fix:** Fix all Critical and High findings. For Medium and Low: fix those with a concrete fix direction stated in the review; skip advisory-only entries. Apply this rule without asking the user.
