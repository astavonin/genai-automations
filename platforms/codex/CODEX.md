...
4. **Observed-failure regression pass:** determine whether the change fixes a failure that actually happened — a red CI job, an on-device or deployment failure, a runtime crash, a manual-testing defect, a bug report, a flake, or any other defect reproduced by running the code, whoever reported it first. If so, require a test reproducing it, at a level matching the failure (composition failures need integration coverage). Check the review request's observed-failure ledger: `covered`, `waived`, and `out-of-scope` entries are resolved — a user-approved waiver is a valid outcome, not a gap. Flag as High: `open` entries, unrecorded failures, an entry carrying two `**Status:**` lines or none (malformed is not resolved), a test that asserts a proxy rather than the observed symptom, a unit test mocking the boundary that *is* the root cause, and an invalid waiver or out-of-scope reason. Flag as Medium: a unit test mocking a crossed boundary that is not the root cause, and a `covered` entry with no red/green evidence. **Exception — external MR reviews:** when the request states no ledger exists because the MR is external, the ledger convention does not apply to that author. Raise a missing regression test as a question, never as a High blocker citing a ledger or waiver they have no mechanism to record.
...
*Cross-platform note: passes 1–3 correspond to the Claude reviewer's Cross-Site Consistency Pass. Pass 1 ≡ Steps 1–3 (enumerate changed contracts at all call sites). Pass 2 ≡ Step 4 — Behavioral extension tracing. Pass 3 ≡ the Composition Failure Coverage test requirement. Pass 4 corresponds to the Claude-side hard gate in `~/.claude/skills/workflows/regression-test/SKILL.md` and its Test Quality Pass Step 3.*
