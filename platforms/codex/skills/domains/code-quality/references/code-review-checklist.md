...
- [ ] Every failure that actually occurred (CI, device, runtime, manual testing, bug report, flake, or any other defect reproduced by running the code, whoever reported it first) has a regression test reproducing it — mandatory, not best-effort. Treat any failure that actually executed as in scope when unsure.
  (`tests/verify-config-consistency.sh` in the genai-automations repo checks that this checkbox's trigger list ends with the run criterion above rather than a review-finding-specific trigger.)
