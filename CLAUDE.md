...
- **`docgate/`** — publishes three commands: `doc-metrics` (per-section prose-word, defensive-register, and design-field metrics for Markdown), `spec-verify` (re-runs an appendix spec's §5 verification rows and checks its §2↔§5 claim graph), and `extract-section` (prints the body of the section one ATX heading opens in one Markdown file; exits 1 — never a silent empty print at exit 0 — on a missing anchor). `pip install -e ./tools/docgate`; `pytest tests/` from that directory. `tests/verify-config-consistency.sh` hard-requires `extract-section` on `PATH`: it exits 1 immediately if the console script is missing, rather than silently skipping the assertions that depend on it. It also hard-requires `shellcheck` on `PATH`, for the same reason — its `fix-mr.md` shell-text assertions pipe extracted blocks to `shellcheck -s bash`.
